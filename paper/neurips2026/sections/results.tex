...
The adaptation stage improves the official validation result, not only the internal diagnostic split. Figure~\ref{fig:adaptation-summary} and Table~\ref{tab:validation-headline} give the direct zero-shot versus fine-tuned comparison required by the project specification: the final tuned model, \texttt{all-linear-r16-seed13}, reaches 84.76\% validation accuracy, compared with 78.72\% for the strongest zero-shot finalist, for an absolute improvement of 6.04 points. The same comparison also improves consensus accuracy by 5.95 points, F1 by 4.50 points, and ROUGE-L by 4.11 points. BLEU is the exception, decreasing slightly from 14.88 to 14.26 despite the exact-match gain; we interpret this as a known limitation of corpus BLEU on short, multi-reference answers rather than as evidence against the main result. Because both systems are evaluated on the same 5,000 validation questions, we also run a paired bootstrap over examples \citep{efron1979bootstrap}. The paired 95\% confidence interval for the accuracy gain is $[5.00, 7.04]$ points, and an exact McNemar test on the discordant decisions \citep{mcnemar1947note} gives $p=1.97\times10^{-32}$.

\begin{table}[t]
    \centering
    \small
    \input{generated/tables/validation_headline_table.tex}
    \caption{Zero-shot versus fine-tuned official validation comparison. Bold entries mark the stronger system-level metric. The paired gain is computed over the same 5,000 validation questions and is therefore a stronger test than comparing independent intervals alone.}
...
\begin{table*}[t]
    ...
    \caption{Post-train internal-dev results for the full LoRA study. Core-matrix rows compare target strategy, rank, and seed. Follow-up rows inherit the winner-conditioned family and probe OCR conditioning or data scaling. Bold entries mark the best metric value in the table, with lower being better for \texttt{eval\_loss}.}
...
\begin{table*}[t]
    ...
    \caption{Validation robustness results from the appendix branch. These runs are intentionally secondary to the main funnel and are used to probe prompt-routing and decoding sensitivity without changing the core winner-selection logic. Bold entries mark the best value in each metric column.}
