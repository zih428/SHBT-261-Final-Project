...
The headline metric is exact-match TextVQA accuracy. In addition, we report secondary semantic and overlap metrics that capture partial-answer quality beyond exact string matching, including BLEU, METEOR, ROUGE-L, token-level F1, precision/recall, and an LLM-as-a-Judge similarity score across all paper-reported experiment tables. The judge score is computed post hoc from saved prediction files using GPT-4.1 mini via the OpenAI API \citep{openai2025gpt41}. Each prediction is compared against the original question and the full set of human reference answers and receives a discrete semantic score in $\{0, 0.5, 1\}$, where $1$ denotes semantic equivalence to at least one acceptable answer, $0.5$ denotes a substantial but incomplete near match, and $0$ denotes a semantic mismatch. These metrics do not replace exact-match accuracy, which remains the primary paper-facing score, but they are useful for understanding whether near-miss generations or paraphrastic answers carry signal that exact matching discards.
