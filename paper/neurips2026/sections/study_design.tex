...
\begin{table*}[t]
    \centering
    \small
    \input{generated/tables/split_audit_table.tex}
...
The data pipeline is fully manifest-based. Preparation scripts materialize the Hugging Face TextVQA records \citep{lhoest2021datasets} into JSONL manifests for train, internal-dev, train-remainder, validation, and test splits, with each row carrying the question id, local image path, question text, reference answers when available, dataset OCR tokens, and optional external OCR tokens. These manifests are the unit consumed by every evaluation and training run, so reruns do not depend on implicit dataset iteration order or ad hoc image lookup. Images are loaded from the manifest path and passed through each backbone's native visual processor whenever possible. Qwen2.5-VL, LLaVA-Phi-3-mini, and BLIP-2 therefore use their Hugging Face Transformers processors \citep{wolf2020transformers} for resizing and tensorization; InternVL2.5 uses its official dynamic tiling recipe with $448{\times}448$ bicubic-resized tiles, thumbnail support, and a configured tile cap.

Text preprocessing follows the same principle of making task assumptions explicit. Prompt variants either hide OCR, expose dataset OCR, expose normalized OCR, or fuse dataset OCR with the external OCR sidecar; OCR-heavy prompts cap the exposed sequence at $32$ tokens to keep prompt length comparable across examples. Reference answers and model predictions are evaluated with a single TextVQA-style normalization path that lowercases text, regularizes punctuation and spacing, removes articles where appropriate, and then computes both exact any-match and the consensus score against the ten human answers. The lexical OCR baseline uses the same manifests, OCR-token selection rules, and answer normalization as the neural models, which keeps it a calibration baseline rather than a separately engineered system.

\subsection{Model pool and prompt settings}

The canonical zero-shot benchmark pool contains four open-source VLM families and one non-neural OCR lexical baseline. The four reportable VLM families are Qwen2.5-VL-3B \citep{qwen2025qwen25vl}, InternVL2.5-4B \citep{chen2024internvl25}, LLaVA-Phi-3-mini \citep{liu2023visual,abdin2024phi3}, and BLIP-2 OPT-2.7B \citep{li2023blip2}. The OCR lexical baseline is included not as a competitor to modern VLMs, but as a calibration point that helps distinguish genuine multimodal reasoning from question-answer overlap driven by visible text alone.
